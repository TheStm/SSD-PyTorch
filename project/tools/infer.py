...
def load_model_and_dataset(args):
    ...
    if dataset_type == 'voc':
        dataset_obj = VOCDataset('test', im_sets=dataset_config['test_im_sets'])
    else:
        dataset_obj = COCODataset('test', coco_root=dataset_config['coco_root'])

    test_dataset = DataLoader(dataset_obj, batch_size=1, shuffle=False)
    ...
    checkpoint_path = os.path.join(train_config['task_name'], f"{train_config['ckpt_name']}")
    assert os.path.exists(checkpoint_path), \
        f"No checkpoint exists at {checkpoint_path}"
    ...
    return model, dataset_obj, test_dataset, config
...
def infer(args):
    ...
    model, dataset_obj, test_dataset, config = load_model_and_dataset(args)
    conf_threshold = config['train_params']['infer_conf_threshold']
    model.low_score_threshold = conf_threshold

    num_samples = 5
    for i in tqdm(range(num_samples)):
        dataset_idx = random.randint(0, len(dataset_obj))
        im_tensor, target, fname = dataset_obj[dataset_idx]
        ...
        for idx, box in enumerate(target['bboxes']):
            ...
            text = dataset_obj.idx2label[target['labels'][idx].detach().cpu().item()]
            text_size, _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_PLAIN, 1, 1)
            text_w, text_h = text_size
            cv2.rectangle(gt_im_copy, (x1, y1), (x1 + 10 + text_w, y1 + 10 + text_h), [255, 255, 255], -1)
            cv2.putText(gt_im, text=dataset_obj.idx2label[target['labels'][idx].detach().cpu().item()],
                        org=(x1 + 5, y1 + 15),
                        thickness=1,
                        fontScale=1,
                        color=[0, 0, 0],
        ...
        for idx, box in enumerate(boxes):
            ...
            text = '{} : {:.2f}'.format(dataset_obj.idx2label[labels[idx].detach().cpu().item()],
...
def evaluate_map(args):
    ...
    for im_tensor, target, fname in tqdm(test_dataset):
        ...
        if 'difficult' in target:
            ...
            difficult = torch.zeros_like(target_labels)
        ...
        for label_name in dataset_obj.label2idx:
        ...
        for idx, box in enumerate(boxes):
            ...
            label_name = dataset_obj.idx2label[label]
    ...
    for idx in range(len(dataset_obj.idx2label)):
        label = dataset_obj.idx2label[idx]
        if label in all_aps:
            print('AP for class {} = {:.4f}'.format(label, all_aps[label]))
